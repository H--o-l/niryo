...
class NiryoOne:
    ...
    class __NiryoOne:
        ...
        def execute_action_non_blocking(self, action_name, action_msg_type, goal):

            self.client = actionlib.ActionClient(action_name, action_msg_type)

            # Connect to server
            # if not self.client.wait_for_server(rospy.Duration(self.action_connection_timeout)):
            #     raise NiryoOneException('Action Server is not up : ' + str(action_name))
